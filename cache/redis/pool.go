...
func (impl *RedisCacheImpl) Publish(channel string, members ...[]byte) (int64, error) {
	if len(members) == 0 {
		return 0, fmt.Errorf("redis publish data empty")
	}
	...
	var vs []interface{}
	vs = append(vs, channel)
	for _, v := range members {
		vs = append(vs, v)
	}
	reply, err := conn.Do("PUBLISH", vs...)

	if err != nil {
		log.Errorf(" key:%s, err:%s", channel, err.Error())
		return 0, err
	} else {
		res := reply.(int64)
		return res, err
...
func (impl *RedisCacheImpl) SScan(key string, count int64) ([][]byte, error) {
	...
	vs := []interface{}{}
	vs = append(vs, key, count)

	reply, err := conn.Do("SSCAN", vs...)
